...
func _ready() -> void:
	owner.connect("squad_leader_changed",Callable(self,"_on_Leader_changed"))
...
func enter(_msg := {}) -> void:
	if ship.is_squad_leader:
		...
		Events.connect("begin_patrol",Callable(self,"_on_SquadLeader_begin_patrol"))
...
func exit() -> void:
	if ship.is_squad_leader:
		...
		Events.disconnect("begin_patrol",Callable(self,"_on_SquadLeader_begin_patrol"))
