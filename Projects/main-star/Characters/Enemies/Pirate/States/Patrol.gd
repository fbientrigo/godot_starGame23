...
func _ready() -> void:
	await owner.ready
	set_behaviors()
	ship.connect("squad_leader_changed",Callable(self,"_on_Leader_changed"))
...
func enter(msg := {}) -> void:
	if ship.is_squad_leader:
		...
		Events.connect("end_patrol",Callable(self,"_on_SquadLeader_end_patrol"))
...
func exit() -> void:
	if ship.is_squad_leader:
		...
		Events.disconnect("end_patrol",Callable(self,"_on_SquadLeader_end_patrol"))
