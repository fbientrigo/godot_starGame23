...
# Does not apply to `RigidBody3D` and `RigidBody2D` nodes.
...
# The percentage between the current linear velocity and 0 to sample by if
# `apply_linear_drag` is true.
# Does not apply to `RigidBody3D` and `RigidBody2D` nodes.
var linear_drag_percentage := 0.0

# The percentage between the current angular velocity and 0 to sample by if
